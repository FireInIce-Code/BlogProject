<template>
  <div class="row bg h-100">
    ...
    <div class="col-9 rightBox" align="center">
      <div class="newBox">
        <b-carousel id="news" :interval="2000" controls indicators>
      ...
      <div align="center" class="bottomBox">
        <h3 class="text-primary">最热博客</h3>
        <div class="hotBlogsBox">
          <router-link
            v-for="blog in newBlogs"
            :key="blog"
            align="left"
            class="blog row"
            v-bind:to="'/blogs/'+blog.id"
            tag="div"
          >
            <div class="col-9">
              <h4
                class="text-primary d-inline"
              >{{blog.title}}&nbsp;&nbsp;&nbsp;&nbsp;[{{blog.date}}]</h4>
            </div>
            <div class="col-3" align="right">
              <h6 class="text-primary d-inline">{{blog.user}}</h6>
            </div>
          </router-link>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getImg(text) {
      ...
      ctx.fillRect(
        parseInt(linelen / 2) * 100 + 5,
        0,
        canvas.width - parseInt(linelen / 2) * 100 - 5,
        canvas.height
      );
      ctx.textAlign = "left";
